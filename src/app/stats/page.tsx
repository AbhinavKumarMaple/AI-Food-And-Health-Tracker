...
import { useCorrelationDataset } from "@/lib/queries";
import { computeStats } from "@/lib/patterns/stats";
import {
  computeCorrelations,
...
import { StatsSkeleton, Skeleton, StatPillSkeleton, RowSkeleton } from "@/components/skeletons";
...
export default function StatsPage() {
  ...
  const data = corrQ.data;

  const stats = useMemo(() => (data ? computeStats(data, range) : null), [data, range]);
  const insights = useMemo(() => {
    if (!data) return [];
    const end = new Date();
    const start = new Date();
    start.setDate(end.getDate() - range);
    return correlationsToInsights(computeCorrelations(data), {
      ...
      <PageHeader
        ...
        right={
          <div className="flex items-center gap-2">
            <RefreshButton loading={corrQ.isFetching} onRefresh={() => corrQ.refetch()} />
            <select
              value={range}
          ...
          <GradientPanel variant="orange" className="p-5">
            ...
            <div className="mt-1 flex items-end justify-between">
              ...
              {stats.moodDelta != null && (
                <span className="rounded-full bg-white/20 px-2.5 py-1 text-[12px] font-bold text-white">{signed(stats.moodDelta)}</span>
              )}
            </div>
